...
$(BUILD_DIR)/.lldb-init: Makefile
	...
	@$(foreach driver,DXECORE DEVICEPATHDXE PCDDXE AMDSEVDXE TDXDXE FVBSERVICESRUNTIMEDXE REPORTSTATUSCODEROUTERRUNTIMEDXE RUNTIMEDXE SECURITYSTUBDXE EBCDXE CPUIO2DXE LOCALAPICTIMERDXE INCOMPATIBLEPCIDEVICESUPPORTDXE PCIHOTPLUGINITDXE RESETSYSTEMRUNTIMEDXE METRONOME HIIDATABASE NULLMEMORYTESTDXE ACPITABLEDXE S3SAVESTATEDXE DPCDXE IOMMUDXE EMUVARIABLEFVBRUNTIMEDXE VARIABLERUNTIMEDXE STATUSCODEHANDLERRUNTIMEDXE PCIHOSTBRIDGEDXE WATCHDOGTIMER SETUPBROWSER SMBIOSDXE QEMUFWCFGACPIPLATFORM BOOTSCRIPTEXECUTORDXE TFTPDYNAMICCOMMAND HTTPDYNAMICCOMMAND LINUXINITRDDYNAMICSHELLCOMMAND LOGODXE QEMURAMFBDXE FAULTTOLERANTWRITEDXE PCRTC MONOTONICCOUNTERRUNTIMEDXE CAPSULERUNTIMEDXE DRIVERHEALTHMANAGERDXE BDSDXE RAMDISKDXE DISPLAYENGINE SMBIOSPLATFORMDXE PLATFORMDXE PCIBUSDXE VIRTIOPCIDEVICEDXE VIRTIO10 VIRTIOBLKDXE VIRTIOSCSIDXE VIRTIORNGDXE VIRTIOSERIALDXE CONPLATFORMDXE CONSPLITTERDXE GRAPHICSCONSOLEDXE TERMINALDXE QEMUKERNELLOADERFSDXE DISKIODXE PARTITIONDXE ENGLISHDXE SCSIBUS SCSIDISK SATACONTROLLER ATAATAPIPASSTHRUDXE ATABUSDXE NVMEXPRESSDXE SIOBUSDXE PCISIOSERIALDXE PS2KEYBOARDDXE BOOTGRAPHICSRESOURCETABLEDXE FAT UDFDXE VIRTIOFSDXE SNPDXE VLANCONFIGDXE MNPDXE ARPDXE DHCP4DXE IP4DXE UDP4DXE MTFTP4DXE TCPDXE UEFIPXEBCDXE ISCSIDXE VIRTIONETDXE UHCIDXE EHCIDXE XHCIDXE USBBUSDXE USBKBDXE USBMASSSTORAGEDXE QEMUVIDEODXE VIRTIOGPUDXE, \
		$(if $(UEFI_DRIVER_$(driver)_DLL), \
			$(if $(UEFI_DRIVER_$(driver)_SLIDE), \
				$(shell echo "# $(driver) driver" >> $@) \
				$(shell echo "# Adding UEFI driver DLL: $(UEFI_DRIVER_$(driver)_DLL)" >> $@) \
				$(shell echo "target modules add $(UEFI_DRIVER_$(driver)_DLL)" >> $@) \
				$(shell echo "# Loading UEFI driver with slide address: $(UEFI_DRIVER_$(driver)_SLIDE)" >> $@) \
				$(shell echo "target modules load --file $(UEFI_DRIVER_$(driver)_DLL) --slide $(UEFI_DRIVER_$(driver)_SLIDE)" >> $@) \
				$(shell echo "" >> $@) \
			) \
		) \
	)
